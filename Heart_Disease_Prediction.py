...
def missing_value(df1):
    ...
    df1["fbs"] = df1["fbs"].fillna(df["fbs"].mode()[0]) # fbs True ya da false turunden bir feature bu yüzden mod alında en çok olan değer yazılır
...
x_result = pd.concat([x_categorical.reset_index(drop= True),x_numerical_imputed_no_outliers.reset_index(drop=True)],axis = 1 )
